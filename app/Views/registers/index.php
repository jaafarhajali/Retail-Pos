      ...
      <table class="table align-middle m-0">
        ...
        <?php foreach ($registers as $r): $isThis = $current !== null && (int) $current['id'] === (int) $r['id']; $s = $inUse[(int) $r['id']] ?? null; ?>
          <tr>
            ...
            <td>
              <?php if ($s !== null): ?>
                <a href="<?= url('sessions/view', ['id' => $s['id']]) ?>"><?= e($s['full_name']) ?></a> <span class="text-muted">· <?= e($s['session_no']) ?> since <?= e(date('d/m H:i', strtotime($s['opened_at']))) ?></span>
              <?php else: ?><span class="text-muted">free</span><?php endif; ?>
            </td>
            <td><?= $r['bound_at'] ? e(date('d/m/Y H:i', strtotime((string) $r['bound_at']))) : '<span class="text-muted">not linked</span>' ?></td>
            <td class="text-end">
        ...
        <?php endforeach; ?>
        <?php if ($registers === []): ?>
          <tr><td colspan="5" class="text-center text-muted py-4">No registers yet.</td></tr>
        <?php endif; ?>
        </tbody>
      </table>
    </div></div>
